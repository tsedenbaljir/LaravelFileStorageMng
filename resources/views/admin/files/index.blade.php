...
{{-- <h3 class="page-title">@lang('quickadmin.files.title')</h3> --}}
    @can('file_create')
        <p>
            @if (Auth::getUser()->role_id == 2 && $userFilesCount >= 5)
                <a href="{{ route('admin.files.create') }}" class="btn btn-success disabled">Шинээр нэмэх</a>
                <a href="{{url('/admin/subscriptions')}}" class="btn btn-primary">Сарын 5000 аар үндсэн багц авах</a>
            @else
                <a href="{{ route('admin.files.create') }}" class="btn btn-success">Файл нэмэх</a>
            @endif
            @if(!is_null(Auth::getUser()->role_id) && config('quickadmin.can_see_all_records_role_id') == Auth::getUser()->role_id)
                @if(Session::get('File.filter', 'all') == 'my')
                    <a href="?filter=all" class="btn btn-default">Бүгдийг харах</a>
                @else
                    <a href="?filter=my" class="btn btn-default">Шүүж харах</a>
                @endif
            @endif
        </p>
    @endcan
    ...
    @can('file_delete')
        ...
        <ul class="list-inline">
            <li><a href="{{ route('admin.files.index') }}" style="{{ request('show_deleted') == 1 ? '' : 'font-weight: 700' }}">Бүх Файл</a></li>
            |
            <li><a href="{{ route('admin.files.index') }}?show_deleted=1" style="{{ request('show_deleted') == 1 ? 'font-weight: 700' : '' }}">Хогийн сав</a></li>
        </ul>
        </p>
    @endcan
                        ...
                        @foreach ($files as $file)
                            <tr data-entry-id="{{ $file->id }}">
                                ...
                                <td field-key='filename'> @foreach($file->getMedia('filename') as $media)
                                    ...
                                    @endforeach</td>
                                <td> @foreach($file->getMedia('filename') as $media)
                                    <p class="form-group"> 
                                        @if($media->size <= 1024*1024)
                                            ({{ $media->size}} KB)
                                        @else   
                                        (<script>
                                             var size= "{{$media->size/1024/1024}}";
                                             document.write(size.substr(0,5));
                                        </script>MB)
                                        @endif
                                    </p>
                                @endforeach</td>
